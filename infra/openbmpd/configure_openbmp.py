#!/usr/bin/python
"""Configure OpenBMP on IOS-XR / IOS-XE host devices."""
import logging, os, sys, napalm
from util import get_hosts
from jinja2 import *

def deploy_openbmp_config(device_type, host_ip):
    ...
    if device_type == 'ios':
        config_file = dirname + '/openbmp_config_xe'
    elif device_type == 'iosxr':
        config_file = dirname + '/openbmp_config_xr'
...
def main():
    ...
    for host in hosts:
        ...
        logging.info('Configuring OpenBMP on %s', host_ip)
        deploy_openbmp_config(device_type, host_ip)
        logging.info('Configured OpenBMP on %s!', host_ip)
        print()
