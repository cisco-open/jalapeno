...
class DefaultApi(object):
    ...
    def get_collectors_with_http_info(self, **kwargs):
        ...
        all_params = ['name', 'description', 'status', 'edge_type', 'field_name', 'timeout', 'last_heartbeat']
        all_params.append('callback')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
        ...
        if 'edge_type' in params:
            query_params['EdgeType'] = params['edge_type']
        if 'field_name' in params:
            query_params['FieldName'] = params['field_name']
        if 'timeout' in params:
            query_params['Timeout'] = params['timeout']
        if 'last_heartbeat' in params:
            query_params['LastHeartbeat'] = params['last_heartbeat']
    ...
    def query_arango(self, collection, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.query_arango(collection, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param str collection: (required)
        :return: object
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.query_arango_with_http_info(collection, **kwargs)
        else:
            (data) = self.query_arango_with_http_info(collection, **kwargs)
            return data

    def query_arango_with_http_info(self, collection, **kwargs):
        """
        query arango for edges and nodes
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.query_arango_with_http_info(collection, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method query_arango" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'collection' is set
        if ('collection' not in params) or (params['collection'] is None):
            raise ValueError("Missing the required parameter `collection` when calling `query_arango`")


        collection_formats = {}

        resource_path = '/query/{Collection}'.replace('{format}', 'json')
        path_params = {}
