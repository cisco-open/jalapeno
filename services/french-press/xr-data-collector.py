...
class Collector(multiprocessing.Process):
    ...
    def run(self):
        ...
        consumer = KafkaConsumer(bootstrap_servers=args.kafkaEndpoint, auto_offset_reset='latest')
        consumer.subscribe(['voltron.Latency'])
        fd = open("router-prefixes.txt", "r")
        prefixes = fd.readlines()
        fd.close()
        for message in consumer:
            latency_data = message.value
            latency_data = ast.literal_eval(latency_data)
