import time
import lsv6_topology_generator, upsert_lsv6_performance as db_upserter
...
def main():
    ...
    while(True):
        lsv6_topology = lsv6_topology_generator.generate_lsv6_topology(arango_client)
        for link_index in range(len(lsv6_topology)):
            current_lsv6_link = lsv6_topology[link_index]
            lsv6_topology_key = current_lsv6_link['key']
            router_igp_id = current_lsv6_link['LocalIGPID']
            router_interface_ip = current_lsv6_link['InterfaceIP']
            router_hostname = lsv6_topology_generator.get_node_hostname(arango_client, router_igp_id)
            interface_name = collect_interface_name(influx_client, router_hostname, router_interface_ip)
            print("\nCalculating performance metrics for Link-State v6 link out of %s(%s) through %s(%s)" % (router_igp_id, router_hostname,
            ...
            upsert_lsv6_performance(arango_client, lsv6_topology_key, calculated_performance_metrics)
            upsert_lsv6_interface_name(arango_client, lsv6_topology_key, interface_name)
            print("============================================================")
        time.sleep(30)
...
def upsert_lsv6_performance(arango_client, lsv6_topology_key, performance_metrics):
    ...
    db_upserter.update_lsv6_performance(arango_client, lsv6_topology_key, in_unicast_pkts, out_unicast_pkts,
...
def upsert_lsv6_interface_name(arango_client, lsv6_topology_key, interface_name):
    db_upserter.update_lsv6_interface_name(arango_client, lsv6_topology_key, interface_name)
