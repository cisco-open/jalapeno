#! /usr/bin/env python
"""AQL Queries executed by the Demo-LSv4_Topology processor"""
...
def get_disjoint_keys(db, ls_topology_keys):
    aql = """ FOR l in Demo-LSLink filter l._key not in @lsv4_topology_keys return l._key """
...
def get_lsv4_link_keys(db):
    aql = """ FOR l in Demo-LSLink filter NOT CONTAINS(l.local_interface_ip, ":") and NOT CONTAINS(l.remote_interface_ip, ":") return l._key """
    bindVars = {}
...
def check_exists_lsv4_link(db, lsv4_topology_key):
    aql = """ FOR l in Demo-LSLink filter l._key == @lsv4_topology_key RETURN { key: l._key } """
...
def createBaseLSv4TopologyDocument(db, ls_link_key):
    aql = """ FOR l in Demo-LSLink filter l._key == @ls_link_key insert l into LSv4_Topology RETURN NEW._key """
    bindVars = {'ls_link_key': ls_link_key}
...
def updateBaseLSv4TopologyDocument(db, ls_link_key):
    aql = """ FOR l in Demo-LSLink filter l._key == @ls_link_key update l into LSv4_Topology RETURN { before: OLD, after: NEW }"""
    bindVars = {'ls_link_key': ls_link_key }
...
def get_srgb_start(db, ls_node_key):
    aql = """ FOR l in Demo-LSNode filter l._key == @ls_node_key return l.srgb_start """
...
def get_max_sid_depth(db, igp_router_id):
    aql = """ FOR l in Demo-LSNode filter l._key == @igp_router_id return l.node_msd """
...
def get_prefix_info(db, igp_router_id):
    aql = """ FOR l in Demo-LSPrefix filter l.igp_router_id == @igp_router_id and l.prefix_sid != null for z in l.prefix_sid filter z.algo == 0 return {"prefix": l.prefix, "length": l.length, "flags": z.flags, "sid_index":  z.prefix_sid}"""
    bindVars = {'igp_router_id': igp_router_id }
...
def get_sr_flags(db, igp_id):
    aql = """ FOR l in Demo-LSPrefix filter l.SRFlags != null and l.IGPRouterID == @igp_id return l.SRFlags[0] """
