#! /usr/bin/env python
"""This class upserts performance metrics into the LSv4_Topology collection in ArangoDB"""
from configs import arangoconfig
from util import connections

def update_lsv4_performance(arango_client, key, in_unicast_pkts, out_unicast_pkts,
    ...
    print("Updating existing LSv4_Topology record " + key + " with performance metrics")
    aql = """FOR p in LSv4_Topology
        FILTER p._key == @key
        UPDATE p with { In_Unicast_Pkts: @in_unicast_pkts, Out_Unicast_Pkts: @out_unicast_pkts,
                        ...
                        Percent_Util_Outbound: @percent_util_outbound } in LSv4_Topology"""
    bindVars = {'key': key,
...
def update_lsv4_interface_name(arango_client, key, interface_name):
    print("Updating existing LSv4_Topology record " + key + " with source interface name: " + interface_name)
    aql = """FOR p in LSv4_Topology
        FILTER p._key == @key
        UPDATE p with { FromInterfaceName: @source_interface_name } in LSv4_Topology"""
